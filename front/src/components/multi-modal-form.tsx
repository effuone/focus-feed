...
import { FileIcon, FileTextIcon, MusicIcon, VideoIcon, XIcon } from 'lucide-react';
  ...
  return (
    <Card className="w-full max-w-4xl">
      <CardHeader>
        ...
        <CardDescription>Select an option to either upload files or provide a URL.</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="file" onValueChange={setActiveTab}>
          ...
          <TabsContent value="url">
            <UrlInputArea urlInput={urlInput} setUrlInput={setUrlInput} />
          </TabsContent>
        </Tabs>
        <Button
          size="lg"
...
const FileUploadArea: React.FC<{
  ...
  <div
    className={`grid gap-2 rounded-md border p-4 transition-colors ${
      isDragOver ? 'bg-muted/50 border-primary' : 'border-neutral-400 hover:border-primary'
    }`}
  ...
  >
    {selectedFiles.length === 0 ? (
      <>
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">or click to select files from your device</p>
        </div>
      </>
    ) : (
          ...
          {selectedFiles.map((file, index) => (
            ...
            >
              <div className="flex items-center space-x-3">
                {filePreviews[index] ? renderFileIcon(filePreviews[index]) : <FileIcon className="h-8 w-8 text-gray-500" />}
    ...
    <Input id="file" type="file" onChange={handleFileChange} className="hidden" multiple />
    <Button
      variant="outline"
      size="sm"
      ...
      <div key={index} className="rounded-lg overflow-hidden shadow-md">
        ...
        {preview.type === 'spreadsheet' && (
          <div className="flex items-center justify-center h-40 bg-gray-100">
            ...
            <span className="ml-2 text-sm font-medium">Spreadsheet Preview not available.</span>
          </div>
        )}
        {preview.type === 'image' && (
          <img src={preview.src} alt={`Preview ${index}`} className="w-full h-40 object-cover" />
        )}
      </div>
    ))}
  </div>
...
function renderFileIcon(preview: FilePreview) {
  ...
  switch (preview.type) {
    ...
    case 'image':
      return preview.src ? (
        <img src={preview.src} alt="Preview" className="h-8 w-8 object-cover rounded" />
      ) : (
